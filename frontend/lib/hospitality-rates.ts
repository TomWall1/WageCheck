...
export async function getHospitalityRates(): Promise<HospitalityRateData> {
  ...
  for (const cls of generalCls) {
    try {
      const rates = await serverFetch<ApiPayRate[]>(
        `/api/award/rates?award=${AWARD_CODE}&classificationId=${cls.id}&employmentType=full_time`
      );
  ...
  if (levelRates.length > 0) {
    ...
    try {
      const rates = await serverFetch<ApiPayRate[]>(
        `/api/award/rates?award=${AWARD_CODE}&classificationId=${generalCls[0]?.id}&employmentType=full_time`
      );
