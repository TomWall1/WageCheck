/**
 * Seed script — Ports, Harbours and Enclosed Water Vessels Award 2020 [MA000052]
 * Pay rates effective 1 July 2025 (following 2025 Annual Wage Review)
 * Rates sourced from FWC MAPD API.
 *
 * Run after migrate.js: node scripts/seed_ma000052.js
 */
require('dotenv').config();
const pool = require('../src/db/pool');

const AWARD_CODE = 'MA000052';
...
async function seed() {
  ...
  try {
    ...
    `, [
      ...
      EFFECTIVE_DATE,
      'https://www.fairwork.gov.au/employment-conditions/awards/awards-summary/ma000052-summary',
    ]);
    ...
    const classifications = [
      {
        ...
        title: 'Crane Driver (under 20 tonnes)',
        description: 'Crane operator operating cranes with a safe working load under 20 tonnes in ports, harbours, or enclosed water vessel operations.',
        duties: ['Operating cranes under 20 tonnes capacity for loading and unloading', 'Conducting pre-start safety checks on crane equipment', 'Following signals from doggers and riggers during lifts', 'Maintaining crane operation logs and records'],
        indicative_tasks: [],
        sort_order: 10,
      },
      {
        level: 1, stream: 'general',
        title: 'Shipkeeper',
        description: 'Employee responsible for the security, watchkeeping, and basic maintenance of vessels while in port or at anchor.',
        duties: ['Maintaining security watch on vessels in port', 'Monitoring mooring lines and vessel condition', 'Performing basic cleaning and upkeep of vessel areas', 'Reporting any safety hazards or incidents to supervisors'],
        indicative_tasks: [],
        sort_order: 20,
      },
      {
        level: 2, stream: 'general',
        title: 'Crane Driver (over 20 tonnes)',
        description: 'Crane operator operating cranes with a safe working load over 20 tonnes in ports, harbours, or enclosed water vessel operations.',
        duties: ['Operating cranes over 20 tonnes capacity for heavy lifts', 'Planning and executing complex crane lifts safely', 'Conducting pre-start and post-operation inspections', 'Coordinating with stevedores and riggers during operations'],
        indicative_tasks: [],
        sort_order: 30,
      },
      {
        level: 2, stream: 'general',
        title: 'General Purpose Hand, Deckhand, Greaser, Passenger Attendant, Turnstile Attendant, Boating Attendant, Host, Fireman, Trimmer, Linesman, Cook, Sailor, Able Seaman, Leading Hand',
        description: 'Crew member performing general vessel or harbour duties including deck work, passenger service, engine room support, and catering on enclosed water vessels.',
        duties: ['Performing deck duties such as line handling and mooring', 'Assisting with passenger boarding, safety briefings, and service', 'Maintaining engine room equipment and performing greasing duties', 'Preparing and serving food and beverages on board vessels', 'Performing general cleaning and maintenance of vessel areas'],
        indicative_tasks: [],
        sort_order: 40,
      },
      {
        ...
        title: 'Engineer',
        description: 'Marine engineer responsible for the operation, maintenance, and repair of vessel propulsion systems and mechanical equipment.',
        duties: ['Operating and maintaining vessel engines and mechanical systems', 'Performing routine servicing and repairs on propulsion equipment', 'Monitoring engine performance gauges and fuel consumption', 'Maintaining engineering logs and reporting defects'],
        indicative_tasks: [],
        sort_order: 50,
      },
      {
        level: 3, stream: 'general',
        title: 'Mate',
        description: 'Second-in-command of a vessel responsible for navigation, safety, and supervising deck crew operations.',
        duties: ['Assisting the Master with navigation and vessel operations', 'Supervising deck crew and coordinating daily work', 'Maintaining safety equipment and conducting drills', 'Overseeing cargo loading and passenger embarkation'],
        indicative_tasks: [],
        sort_order: 60,
      },
      {
        ...
        title: 'Master',
        description: 'Person in command of a vessel with ultimate responsibility for navigation, safety of the vessel, crew, and passengers.',
        duties: ['Commanding the vessel and making all navigational decisions', 'Ensuring compliance with maritime safety regulations', 'Managing crew and overseeing all vessel operations', 'Maintaining vessel logs and communicating with port authorities'],
        indicative_tasks: [],
        sort_order: 70,
      },
    ];
    ...
    const allowances = [
      {
        allowance_type: 'compensation_for_loss_of_personal_effect',
        name: 'Compensation for loss of personal effects—an amount of up to',
        description: 'Compensation for loss of personal effects—an amount of up to',
        trigger_condition: 'As per award conditions',
        amount: 2362, amount_type: 'fixed', per_unit: 'per_shift',
      },
      {
        allowance_type: 'meal',
        name: 'Meal allowance—overtime in excess of one and a half hours',
        description: 'Meal allowance—overtime in excess of one and a half hours',
        trigger_condition: 'As per award conditions',
        amount: 21.44, amount_type: 'fixed', per_unit: 'per_shift',
      },
      {
        allowance_type: 'meal',
        name: 'Meal allowances—further 4 hours of overtime',
        description: 'Meal allowances—further 4 hours of overtime',
        trigger_condition: 'As per award conditions',
        amount: 21.44, amount_type: 'fixed', per_unit: 'per_shift',
      },
      {
        allowance_type: 'vehicle_car',
        name: 'Transport allowance—own vehicle',
        description: 'Transport allowance—own vehicle',
        trigger_condition: 'As per award conditions',
        amount: 0.98, amount_type: 'per_km', per_unit: 'per_km',
      },
      {
        allowance_type: 'vehicle_car',
        name: 'Transport allowance—own vehicle—travel to or from different starting or finishing point',
        description: 'Transport allowance—own vehicle—travel to or from different starting or finishing point',
        trigger_condition: 'As per award conditions',
        amount: 0.98, amount_type: 'per_km', per_unit: 'per_km',
      },
      {
        allowance_type: 'uniform',
        name: 'Uniforms allowance',
        description: 'Uniforms allowance',
        trigger_condition: 'As per award conditions',
        amount: 18.3, amount_type: 'weekly', per_unit: 'per_week',
      },
    ];
    ...
    const questions = [
      {
        ...
        sort_order: 1,
        answers: [
          { answer_key: 'level_1', answer_text: 'Crane Driver (under 20 tonnes)', sort_order: 1 },
          { answer_key: 'level_1', answer_text: 'Shipkeeper', sort_order: 2 },
          { answer_key: 'level_2', answer_text: 'Crane Driver (over 20 tonnes)', sort_order: 3 },
          { answer_key: 'level_2', answer_text: 'General Purpose Hand, Deckhand, Greaser, Passenger Attendant, Turnstile Attendant, Boating Attendant, Host, Fireman, Trimmer, Linesman, Cook, Sailor, Able Seaman, Leading Hand', sort_order: 4 },
          { answer_key: 'level_3', answer_text: 'Engineer', sort_order: 5 },
          { answer_key: 'level_3', answer_text: 'Mate', sort_order: 6 },
          { answer_key: 'level_4', answer_text: 'Master', sort_order: 7 },
        ],
      },
    ];
