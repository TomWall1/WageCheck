/**
 * Seed script — Ports, Harbours and Enclosed Water Vessels Award 2020 [MA000015]
 * Pay rates effective 1 July 2025
 * Source: FWC MAPD API + FWO pay guide MA000015
 *
 * One stream:
 *   ports — Levels 1–5 (5 classifications)
 *
 * Adult FT/PT hourly rates:
 *   L1: $27.52   L2: $28.58   L3: $29.92   L4: $31.42   L5: $33.18
 *
 ...
 *
 * Allowances:
 *   Meal: $20.96 (overtime > 2 hrs)
 *
 * Run after migrate.js: node scripts/seed_ma000015.js
 */
require('dotenv').config();
const pool = require('../src/db/pool');

const AWARD_CODE = 'MA000015';
...
async function seed() {
  ...
  try {
    ...
    `, [
      ...
      EFFECTIVE_DATE,
      'https://awards.fairwork.gov.au/MA000015.html',
    ]);

    // ── Classifications ────────────────────────────────────────────────────────
    const classifications = [
      {
        level: 1, stream: 'ports',
        fwc_id: null,
        title: 'Ports Level 1',
        description: 'Entry-level waterfront worker. You perform routine tasks under direct supervision, including basic cargo handling and general wharf duties.',
        duties: [
          'Performing general labouring duties on wharves and in port areas',
          'Assisting with loading and unloading cargo under supervision',
          'Maintaining cleanliness and safety of work areas',
          'Following established procedures for vessel mooring and unmooring',
        ],
        indicative_tasks: ['Waterfront worker (entry)', 'Wharf labourer', 'Cargo handler (basic)'],
        sort_order: 10,
      },
      {
        level: 2, stream: 'ports',
        fwc_id: null,
        title: 'Ports Level 2',
        description: 'Experienced waterfront worker with basic equipment skills. You perform a range of port operations with limited supervision.',
        duties: [
          'Operating basic equipment such as hand trucks and pallet jacks',
          'Securing cargo using lashing and restraint systems',
          'Assisting with vessel berthing and departure procedures',
          'Completing documentation for cargo receival and dispatch',
        ],
        indicative_tasks: ['Experienced waterfront worker', 'Cargo processor', 'Vessel assistant'],
        sort_order: 20,
      },
      {
        level: 3, stream: 'ports',
        fwc_id: null,
        title: 'Ports Level 3',
        description: 'Skilled operator with crane and forklift capabilities. You operate specialised port equipment and work with minimal supervision.',
        duties: [
          'Operating forklifts, cranes, and other heavy lifting equipment in port areas',
          'Conducting pre-start checks and routine maintenance on equipment',
          'Coordinating cargo movements within the terminal or harbour',
          'Ensuring safe stowage and securing of cargo on vessels',
        ],
        indicative_tasks: ['Crane operator', 'Forklift operator (port)', 'Terminal equipment operator'],
        sort_order: 30,
      },
      {
        level: 4, stream: 'ports',
        fwc_id: null,
        title: 'Ports Level 4',
        description: 'Advanced operator or tradesperson. You operate complex equipment, hold trade qualifications, or perform specialised technical duties.',
        duties: [
          'Operating large-scale container cranes, ship-to-shore cranes, and straddle carriers',
          'Performing trade-level maintenance on port equipment and infrastructure',
          'Training and mentoring junior workers on equipment operation and safety',
          'Managing cargo planning and terminal logistics coordination',
        ],
        indicative_tasks: ['Ship-to-shore crane operator', 'Port tradesperson', 'Terminal logistics coordinator'],
        sort_order: 40,
      },
      {
        level: 5, stream: 'ports',
        fwc_id: null,
        title: 'Ports Level 5',
        description: 'Leading hand or supervisor. You supervise a team or manage a specific operational area within the port or harbour.',
        duties: [
          'Supervising waterfront workers and coordinating shift operations',
          'Managing scheduling and resource allocation for port operations',
          'Ensuring compliance with maritime safety and environmental regulations',
          'Liaising with shipping agents, vessel masters, and port authorities',
        ],
        indicative_tasks: ['Leading hand', 'Shift supervisor', 'Operations coordinator (port)'],
        sort_order: 50,
      },
    ];
    ...
    const baseRates = {
      ports: { 1: 27.52, 2: 28.58, 3: 29.92, 4: 31.42, 5: 33.18 },
    };
    ...
    //   Q1: ports_level — level 1 through 5
    const questions = [
      {
        question_key: 'ports_level',
        question_text: 'What is your ports and harbours classification level?',
        help_text: 'Your level depends on your experience, qualifications, and the complexity of your duties. Level 1 is entry-level waterfront work. Higher levels involve operating cranes and forklifts, trade qualifications, and supervising teams.',
    ...
    const answers = [
      { question_key: 'ports_level', answer_key: 'l1', answer_text: 'Level 1 — Entry-level (waterfront worker, wharf labourer)', sort_order: 1 },
      { question_key: 'ports_level', answer_key: 'l2', answer_text: 'Level 2 — Experienced (basic equipment operator, cargo processor)', sort_order: 2 },
      { question_key: 'ports_level', answer_key: 'l3', answer_text: 'Level 3 — Skilled operator (crane, forklift, terminal equipment)', sort_order: 3 },
      { question_key: 'ports_level', answer_key: 'l4', answer_text: 'Level 4 — Advanced operator/tradesperson (ship-to-shore crane, port trades)', sort_order: 4 },
      { question_key: 'ports_level', answer_key: 'l5', answer_text: 'Level 5 — Leading hand/supervisor (shift supervisor, operations coordinator)', sort_order: 5 },
    ];
