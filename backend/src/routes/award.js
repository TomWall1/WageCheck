...
// Load the valid-award set from the DB at startup. Cached for the lifetime
// of the process; restart required to pick up newly seeded awards (same
// constraint as loading rates — seeds are applied out of band from the API).
const VALID_AWARDS = new Set();
(async () => {
  try {
    const r = await pool.query('SELECT DISTINCT award_code FROM classifications');
    for (const row of r.rows) VALID_AWARDS.add(row.award_code);
    console.log(`[VALID_AWARDS] loaded ${VALID_AWARDS.size} awards from DB`);
  } catch (err) {
    console.warn('[VALID_AWARDS] failed to load from DB — defaulting to allow-only MA000009:', err.message);
    VALID_AWARDS.add(DEFAULT_AWARD);
  }
})();

function getAwardCode(req) {
  const code = req.query.award || req.body?.award || DEFAULT_AWARD;
  return VALID_AWARDS.has(code) ? code : DEFAULT_AWARD;
}
