/**
 * Poultry Processing Award Test Runner — MA000074
 * Run: node testing/run_poultry_tests.js
 *
 * Key award features:
 *   - Stream: poultry, 6 levels
...
const t = createTestRunner('MA000074', path.join(__dirname, 'Poultry_Testing_Plan.xlsx'));
...
const lvl = (level) => classId(level, 'poultry');
...
async function runBaseRateTests() {
  ...
  // FT/PT base rates
  console.log('\n1.1 Poultry FT/PT base rates');
  for (const b of [
    { id: 'BR-01', level: 1, expected: 25.06, title: 'L1 FT' },
    { id: 'BR-02', level: 2, expected: 25.73, title: 'L2 FT' },
    { id: 'BR-03', level: 3, expected: 26.07, title: 'L3 FT' },
    { id: 'BR-04', level: 4, expected: 26.41, title: 'L4 FT' },
    { id: 'BR-05', level: 5, expected: 26.74, title: 'L5 FT' },
    { id: 'BR-06', level: 6, expected: 27.43, title: 'L6 FT' },
  ]) {
  ...
  console.log('\n1.2 Poultry casual rates (×1.25)');
  for (const c of [
    { id: 'CL-01', level: 1, ft: 25.06, title: 'L1 casual' },
    { id: 'CL-02', level: 2, ft: 25.73, title: 'L2 casual' },
    { id: 'CL-03', level: 3, ft: 26.07, title: 'L3 casual' },
    { id: 'CL-04', level: 4, ft: 26.41, title: 'L4 casual' },
    { id: 'CL-05', level: 5, ft: 26.74, title: 'L5 casual' },
    { id: 'CL-06', level: 6, ft: 27.43, title: 'L6 casual' },
  ]) {
...
async function runPenaltyRateTests() {
  ...
  const ftBase = 25.06; // L1 FT
  const casBase = round2(25.06 * 1.25); // L1 casual
...
async function runAllowanceTests() {
  ...
  try {
    ...
    for (const row of r.rows) {
      ...
      if (row.allowance_type === 'first_aid') record('AL-02', 21.41, amt, 'First aid allowance');
      if (row.allowance_type === 'cold_work_mid') record('AL-03', 1.67, amt, 'Cold work (mid range) allowance');
    }
...
async function runComplexTests() {
  ...
  const ftBase = 25.06;
  const casBase = round2(25.06 * 1.25);
  ...
  try {
    ...
    record('CS-06', round2(round2(25.06 * 0.60) * 1.25), r.baseHourlyRate, 'Casual L1 age 17 (60% then ×1.25)');
...
async function runRegressionTests() {
  ...
  try {
    ...
    record('RT-01', round2(26.07 * 4), payOnly(r), 'L3 FT Mon 4hr = $26.07 × 4');
  ...
  try {
    ...
    record('RT-02', round2(round2(26.74 * 1.25) * 4), payOnly(r), 'L5 casual Mon 4hr');
  ...
  try {
    ...
    record('RT-03', round2(25.06 * 2.00 * 7.5), payOnly(r), 'L1 FT Sunday 7.5hr (×2.00)');
  ...
  try {
    ...
    record('RT-04', round2(round2(25.73 * 1.25) * 2.25 * 4), payOnly(r), 'L2 casual PH 4hr (×2.25)');
  ...
  try {
    ...
    record('RT-05', round2(26.41 * 1.50 * 4), payOnly(r), 'L4 FT Saturday 4hr (×1.50)');
...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════════════════ */
async function main() {
  console.log('WageCheck Poultry Processing Award Test Runner — MA000074');
